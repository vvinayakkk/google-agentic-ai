...
prompt = ChatPromptTemplate.from_messages([
    ("system", (
        ...
        "boundingBoxes (always a list of objects, each with x, y, width, height as floats, all in percent, 0-100, relative to the fixed image size, covering only the affected/visible diseased area, not the whole image. If only one region, return a single box in a list. If healthy, return a single box covering the whole image [{{x:0, y:0, width:100, height:100}}]. Never return empty or null.), "
        "boundingBoxExplanation (short, how you determined the boxes), "
        "description (detailed, include ambiguity if multiple diseases possible), "
        "symptoms (list), "
        "solutions (list of dicts: title, details, actionable). "
        "If ambiguous, mention this in description and pick the most likely. "
        "Be as accurate as possible. Always follow this output format.\n\n"
        + FEW_SHOT_EXAMPLES +
        "\n\n{format_instructions}"
    )),
    ("human", [
