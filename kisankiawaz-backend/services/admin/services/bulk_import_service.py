            ...
            rows.append(
                {
                    ...
                    "source_type": str(provider.get("source_type") or "").strip(),
                    "source_url": str(provider.get("source_url") or "").strip(),
                    "source": str(provider.get("source") or "equipment_pan_india_curated_2026").strip(),
                    "state": state,
                    "district": district,
                    "city": str(provider.get("city") or district).strip(),
                    "pincode": str(provider.get("pincode") or "").strip(),
                    "address": str(provider.get("address") or "").strip(),
                    "contact_person": str(provider.get("contact_person") or "").strip(),
                    "provider_phone": str(provider.get("contact_number") or provider.get("phone") or "").strip(),
                    "contact_number": str(provider.get("contact_number") or provider.get("phone") or "").strip(),
                    "alternate_phone": str(provider.get("alternate_contact") or "").strip(),
                    "alternate_contact": str(provider.get("alternate_contact") or "").strip(),
                    "whatsapp": str(provider.get("whatsapp") or provider.get("contact_number") or provider.get("phone") or "").strip(),
                    "eligibility": provider.get("eligibility") or [],
                    "documents_required": provider.get("documents_required") or [],
                    "security_deposit_policy": str(provider.get("security_deposit_policy") or "").strip(),
                    "booking_channels": _split_multi_value(provider.get("booking_channels")),
                    "working_hours": str(provider.get("working_hours") or "").strip(),
                    "service_radius_km": _as_int(provider.get("service_radius_km")),
                    "geo": provider.get("geo") if isinstance(provider.get("geo"), dict) else {},
                    "name": equipment_name,
                    "category": str(item.get("category") or "").strip(),
                    "equipment_id": f"equip_{_slug(equipment_name)}_{_slug(state)}_{_slug(district)}",
                    "location_key": f"{_slug(state)}-{_slug(district)}",
                    ...
                    "last_verified_at": str(provider.get("last_verified_at") or "").strip(),
                    "is_active": True,
                    "_ingested_at": now_iso,
                    "_ingestion_source": "admin_bulk_import",
                }
