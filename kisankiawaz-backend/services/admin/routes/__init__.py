...
async def admin_login(body: AdminLoginRequest):
    ...
    db = get_firestore()
    query = db.collection(Firestore.ADMIN_USERS).where("email", "==", body.email).limit(1)
    ...
    await db.collection(Firestore.ADMIN_USERS).document(doc.id).update({"last_login_at": datetime.now(timezone.utc).isoformat()})
...
async def get_stats(admin: dict = Depends(get_current_admin)):
    ...
    snap = await db.collection(Firestore.ANALYTICS_SNAPSHOTS).document(today).get()
    if snap.exists:
        return snap.to_dict()
    # Fallback: count farmers
    total = 0
    async for _ in db.collection(Firestore.USERS).where("role", "==", "farmer").stream():
        total += 1
...
async def get_data_freshness(admin: dict = Depends(get_current_admin)):
    ...
    db = get_firestore()
    items = []
    async for doc in db.collection(Firestore.REF_DATA_INGESTION_META).stream():
        data = doc.to_dict()
...
async def list_farmers(
    ...
    db = get_firestore()
    query = db.collection(Firestore.USERS).where("role", "==", "farmer")
    docs = []
...
async def get_farmer(farmer_id: str, admin: dict = Depends(get_current_admin)):
    """Full farmer profile."""
    db = get_firestore()
    doc = await db.collection(Firestore.USERS).document(farmer_id).get()
    if not doc.exists:
    ...
    data.pop("password_hash", None)
    # Get profile
    profile = await db.collection(Firestore.FARMER_PROFILES).document(f"profile_{farmer_id}").get()
    if profile.exists:
...
async def update_farmer_status(farmer_id: str, body: FarmerStatusUpdate, admin: dict = Depends(get_current_admin)):
    ...
    if not doc.exists:
        raise not_found("Farmer not found")
    await db.collection(Firestore.USERS).document(farmer_id).update({"is_active": body.is_active, "updated_at": datetime.now(timezone.utc).isoformat()})
    # Audit log
    await db.collection(Firestore.ADMIN_AUDIT_LOGS).add({"admin_id": admin.get("id", ""), "action": "UPDATE_FARMER_STATUS", "target_collection": "users", "target_doc_id": farmer_id, "payload_summary": f"is_active={body.is_active}", "timestamp": datetime.now(timezone.utc).isoformat()})
...
async def get_farmer_conversations(farmer_id: str, admin: dict = Depends(get_current_admin)):
    ...
    db = get_firestore()
    convos = []
    async for doc in db.collection(Firestore.AGENT_CONVERSATIONS).where("user_id", "==", farmer_id).stream():
        data = doc.to_dict()
        data["id"] = doc.id
        convos.append(data)
...
async def list_schemes(page: int = Query(1, ge=1), per_page: int = Query(20, ge=1, le=100), admin: dict = Depends(get_current_admin)):
    db = get_firestore()
    docs = []
    async for doc in db.collection(Firestore.REF_FARMER_SCHEMES).stream():
        data = doc.to_dict()
        data["id"] = doc.id
        docs.append(data)
    total = len(docs)
...
async def create_scheme(data: dict, admin: dict = Depends(get_current_admin)):
    ...
    if doc_id:
        await db.collection(Firestore.REF_FARMER_SCHEMES).document(doc_id).set(data, merge=True)
    else:
        _, ref = await db.collection(Firestore.REF_FARMER_SCHEMES).add(data)
        doc_id = ref.id
    await db.collection(Firestore.ADMIN_AUDIT_LOGS).add({"admin_id": admin.get("id", ""), "action": "CREATE_SCHEME", "target_collection": "ref_farmer_schemes", "target_doc_id": doc_id, "timestamp": now})
...
async def update_scheme(scheme_id: str, data: dict, admin: dict = Depends(get_current_admin)):
    ...
    await db.collection(Firestore.REF_FARMER_SCHEMES).document(doc_id).update(data)
...
async def delete_scheme(scheme_id: str, admin: dict = Depends(get_current_admin)):
    db = get_firestore()
    doc_id = f"scheme_{scheme_id}" if not scheme_id.startswith("scheme_") else scheme_id
    await db.collection(Firestore.REF_FARMER_SCHEMES).document(doc_id).update({"is_active": False})
...
async def list_providers(admin: dict = Depends(get_current_admin)):
    db = get_firestore()
    items = []
    async for doc in db.collection(Firestore.REF_EQUIPMENT_PROVIDERS).stream():
        data = doc.to_dict()
        data["id"] = doc.id
        items.append(data)
...
async def create_provider(data: dict, admin: dict = Depends(get_current_admin)):
    ...
    if rental_id:
        await db.collection(Firestore.REF_EQUIPMENT_PROVIDERS).document(rental_id).set(data, merge=True)
    else:
        await db.collection(Firestore.REF_EQUIPMENT_PROVIDERS).add(data)
...
async def update_provider(rental_id: str, data: dict, admin: dict = Depends(get_current_admin)):
    db = get_firestore()
    doc = await db.collection(Firestore.REF_EQUIPMENT_PROVIDERS).document(rental_id).get()
    if not doc.exists:
        raise not_found("Provider not found")
    await db.collection(Firestore.REF_EQUIPMENT_PROVIDERS).document(rental_id).update(data)
...
async def browse_mandi_prices(
    ...
    db = get_firestore()
    query = db.collection(Firestore.REF_MANDI_PRICES)
    if state:
...
async def trigger_ingestion(script_name: str, admin: dict = Depends(get_current_super_admin)):
    ...
    db = get_firestore()
    now = datetime.now(timezone.utc).isoformat()
    await db.collection(Firestore.ADMIN_AUDIT_LOGS).add({
...
async def get_config(admin: dict = Depends(get_current_admin)):
    db = get_firestore()
    doc = await db.collection(Firestore.APP_CONFIG).document("global").get()
    if doc.exists:
        return doc.to_dict()
...
async def update_config(body: AppConfigUpdate, admin: dict = Depends(get_current_super_admin)):
    ...
    await db.collection(Firestore.APP_CONFIG).document("global").update(updates)
    await db.collection(Firestore.ADMIN_AUDIT_LOGS).add({"admin_id": admin.get("id", ""), "action": "UPDATE_CONFIG", "target_collection": "app_config", "target_doc_id": "global", "payload_summary": str(updates), "timestamp": updates["updated_at"]})
...
async def update_feature_flags(flags: dict, admin: dict = Depends(get_current_super_admin)):
    db = get_firestore()
    now = datetime.now(timezone.utc).isoformat()
    await db.collection(Firestore.APP_CONFIG).document("global").update({"feature_flags": flags, "updated_at": now, "updated_by": admin.get("id", "")})
...
async def list_admins(admin: dict = Depends(get_current_super_admin)):
    db = get_firestore()
    items = []
    async for doc in db.collection(Firestore.ADMIN_USERS).stream():
        data = doc.to_dict()
        data["id"] = doc.id
        data.pop("password_hash", None)
        items.append(data)
...
async def create_admin(body: AdminUserCreate, admin: dict = Depends(get_current_super_admin)):
    db = get_firestore()
    # Check duplicate
    existing = db.collection(Firestore.ADMIN_USERS).where("email", "==", body.email).limit(1)
    ...
    await db.collection(Firestore.ADMIN_USERS).document(admin_id).set(admin_doc)
...
async def update_admin_status(admin_id: str, body: FarmerStatusUpdate, admin: dict = Depends(get_current_super_admin)):
    db = get_firestore()
    doc = await db.collection(Firestore.ADMIN_USERS).document(admin_id).get()
    if not doc.exists:
        raise not_found("Admin not found")
    await db.collection(Firestore.ADMIN_USERS).document(admin_id).update({"is_active": body.is_active})
