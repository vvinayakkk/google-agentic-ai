...
export default function LiveVoiceScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(`🔍 Testing connection to: http://192.168.0.107:8001`);
      ...
      const response = await axios.post(`http://192.168.0.107:8001/agent`, testPayload, { 
        timeout: 5000,
      ...
      if (response.status === 200) {
        ...
        try {
          ...
          const audioTestResponse = await axios.post(`http://192.168.0.107:8001/audio_agent`, testFormData, { 
            timeout: 5000,
  ...
  const processVoiceCommand = async (audioUri) => {
    ...
    try {
      ...
      console.log(`🎤 Making request to: http://192.168.0.107:8001/audio_agent`);
      console.log(`📋 Form data keys:`, Array.from(formData.keys()));
      
      const response = await axios.post(`http://192.168.0.107:8001/audio_agent`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      ...
      if (error.response && error.response.status === 404) {
        ...
        try {
          ...
          const fallbackResponse = await axios.post(`http://192.168.0.107:8001/agent`, fallbackPayload, {
            headers: { 'Content-Type': 'application/json' },
            timeout: 30000,
          });
                        ...
                        <TouchableOpacity 
                          ...
                          onPress={async () => {
                            try {
                              ...
                              const response = await axios.post(`http://192.168.0.107:8001/generate_audio`, {
